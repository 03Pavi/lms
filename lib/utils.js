...
function generate_pagination_info(page, limit, sort_column, sort_order) {
  ...
  sort_order = orders.includes(sort_order.toLowerCase())
...
function genrate_sessions(settings) {
  let { sessions_amount, sessions_per_week, hours_per_session, group_id } =
    settings;
...
function update_attendance_with_uuid(attendances = [], students = []) {
  ...
  students.forEach((student) => {
    if (
      ...
      student.dataValues.uuid !== undefined
    ) {
...
function get_gcp_content_type({ file_extension }) {
  ...
  return (
    mime.lookup(file_extension.toLowerCase()) || "application/octet-stream"
  );
}
