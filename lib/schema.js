...
const leave_applicability_object_schema = z.object({
    ...
    if (data.value === null && data.user_id === null) {
        ctx.addIssue({
        path: ['value'],
        message: 'Either value or user_id must be present',
        });
        ctx.addIssue({
        path: ['user_id'],
        message: 'Either value or user_id must be present',
    ...
    if (data.value !== null && data.user_id !== null) {
        ctx.addIssue({
        path: ['value'],
        message: 'If value is present, user_id must be null',
        });
        ctx.addIssue({
        path: ['user_id'],
        message: 'If user_id is present, value must be null',
        });
    }
});
...
const leave_restriction_schema = z.object({
    ...
    clubbed_leaves: z.array(z.string().uuid().optional())
});
